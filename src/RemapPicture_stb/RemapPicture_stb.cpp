...
int resavePng(const UString& a_sDirName)
{
	...
	for (vector<UString>::iterator it = vPngFileName.begin(); it != vPngFileName.end(); ++it)
	{
		...
		u32 uPngHeight = 0;
		n32 nPngChannel = 0;
		u8* pTemp = stbi_load_from_file(fp, reinterpret_cast<n32*>(&uPngWidth), reinterpret_cast<n32*>(&uPngHeight), &nPngChannel, 4);
		fclose(fp);
		if (pTemp == nullptr)
		{
			return 1;
		...
		memcpy(pData, pTemp, uPngWidth * uPngHeight * 4);
		stbi_image_free(pTemp);
